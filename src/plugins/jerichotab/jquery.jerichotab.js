...
$.extend($.fn, {
    initJerichoTab: function(setting) {
        ...
        function createJerichoTab() {
            ...
            $.fn.jerichoTab = {
                ...
                tabPageWidth: $('.tab_pages', jerichotab).width()-40,
                ...
                addTab: function(tabsetting) {
                    ...
                    if(ps.tabFirer != null) {
                        ...
                        var mask = ps.tabFirer.attr('id');
                        if(typeof mask!='undefined'&&$("[mask="+mask+"]").length>0){
                            return $.fn.setTabActive(mask).adaptSlider().loadData();
                        }
                    }
                    // set name
                    if(ps.name == ''){
                    ...
                    /*var newTab = $('<li class="jericho_tabs tab_selected" style="width:0px"  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
                     ...
                     '<div class="tab_right">&nbsp;</div>' +
                     '</li>')*/
                    var newTab = $('<li name="'+ps.name+'" class="jericho_tabs tab_selected" style="width:0px" mask="'+$(ps.tabFirer).attr('id')+'"  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
                    '<div class="tab_left"  style="width:' + (opts.tabWidth - 5) + 'px"  >' +
                    (ps.iconImg == '' ? '' : '<div class="tab_icon" style="' + 'background-image:url(' + ps.iconImg + ')' + '">&nbsp;</div>') +
                    '<div class="tab_text" title="' + ps.title + '"  style="width:' + (opts.tabWidth - 45 + (ps.iconImg == '' ? 25 : 0)) + 'px"  >' + ps.title.cut(opts.tabWidth / 10 - 1) + '</div>  ' +
                    '<div class="tab_close">' + (ps.closeable ? '<a href="javascript:" title="关闭">&nbsp;</a>' : '') + '</div>' +
                    ...
                    '</li>').appendTo($.fn.jerichoTab.tabpage).css('opacity', '0').applyHover()
                        .applyCloseEvent().animate({ 'opacity': '1', width: opts.tabWidth }, 100, function() {
                            $.fn.setTabActive(curIndex).loadData();
                        });
    ...
    setTabActive: function(orderKey) {
        ...
        //解决chrome浏览器高版本，在切换tab，iframe滚动条消失的问题
        //var ifameHight = $(".jericho_tab iframe").height()-1;
        //$(".jericho_tab iframe").height(ifameHight);
        //var timer = setTimeout(function() {
        //    clearTimeout(timer)
        //    $(".jericho_tab iframe").height(ifameHight+1);
        //},30)

        if(typeof orderKey == "number") {
            return $('#jerichotab_' + orderKey).swapTabEnable();
        }else {
            return $("[mask="+orderKey+"]").swapTabEnable();
        }
    ...
    buildIFrame: function(src, holder,flag) {
        return this.each(function() {
            ...
            src += (src.indexOf('?') == -1 ? '?' : '&') + 'tabPageId=' + jerichotabiframe+'&&flag='+flag;
            var iframe = $('<iframe id="' + jerichotabiframe + '" name="' + jerichotabiframe + '" src="' + src + '" frameborder="0" scrolling="auto" />')
                .css({ width: '100%', height: '100%',border: 0}).appendTo($(this));
            ...
            function load(c,flag) {
                switch (dataType) {
                    ...
                    case 'iframe':
                        ...
                        holder.buildIFrame(dataLink, holder,flag);
                        break;
                    case 'ajax':
                    ...
                    for (var t in tabHash) {
                        if (tabHash[t].tabId == $(me).attr('id')) {
                            if (tabHash[t].tabFirer != null)
                                //tabHash[t].tabFirer[0].removeAttr('jerichotabindex');
                            tabHash.splice(t, 1);
                        }
                    }
                    //adapt slider
